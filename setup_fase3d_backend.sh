...
# bash setup_fase3d_backend.sh

echo "🍫 Configurando backend Fase 3D..."

# ── operacionalController.js ─────────────────────────────────
cat > src/controllers/operacionalController.js << 'EOF'
...
# ── routes/operacional.js ────────────────────────────────────
cat > src/routes/operacional.js << 'EOF'
...
# ── routes/suporte.js — para clientes e lojas ────────────────
cat > src/routes/suporte.js << 'EOF'
const express = require('express');
const router  = express.Router();
...
echo "✅ Backend Fase 3D configurado!"
git add .
git commit -m "feat: logs, inativos, exportação CSV e suporte"
git push
echo "✅ Push feito!"
