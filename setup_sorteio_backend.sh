...
# bash setup_sorteio_backend.sh

echo "🍫 Configurando backend — Coins Admin + Sorteio..."

# ── adminCoinsController.js ──────────────────────────────────
cat > src/controllers/adminCoinsController.js << 'EOF'
...
cat > src/routes/adminCoins.js << 'EOF'
...
# ── server.js — adiciona rota adminCoins ─────────────────────
...
git add .
git commit -m "feat: coins admin, envio e sorteio"
git push
echo "✅ Push feito!"